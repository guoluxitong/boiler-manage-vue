<template>
    <el-dialog
      ...
      <el-form ref="productForm" :model="formData" label-position="right" label-width="100px" style='width: 96%; margin-left:15px;'>
        <el-table :data="formData.productAuxiliaryMachineInfoList" element-loading-text="给我一点时间" border style="margin-top:10px" @row-contextmenu="openTableMenu">
  ...
  export default {
    ...
    methods:{
      auxiliaryMachineClose(){
        this.$emit('auxiliaryMachineDialogClose',{auxiliaryMachineDialogVisible:false})
      },
      ...
      confirmAuxiliaryMachineInfoDialog(obj){
        if(obj.flag){
          this.auxiliaryMachineInfoDialogVisible=obj.auxiliaryMachineInfoDialogVisible
          if (obj.title==='编辑'){
            this.handleDelete(obj.productAuxiliaryMachineInfo)
          }
          this.formData.productAuxiliaryMachineInfoList.push(obj.auxiliaryMachineInfoFormData)
      ...
      submitForm() {
        this.$refs.productForm.validate(valid => {
          if (valid) {
            ...
            this.$emit('confirmAuxiliaryMachineDialog',{auxiliaryMachineDialogVisible:false,flag:true,productFormData:this.formData})
          } else {
            return false
          }
        })
