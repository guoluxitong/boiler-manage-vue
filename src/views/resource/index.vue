<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="页面地址">
        <template slot-scope="scope">
          ...
          <span
            v-else
          >{{(scope.row.pageUrl==''||scope.row.pageUrl==null)?'/'+scope.row.url+'/'+'index':scope.row.pageUrl}}</span>
        </template>
      </el-table-column>
    ...
    <menu-context ref="menuContext">
      <span :style="{display:isShowAddChildMenu}">
        <menu-context-item @click="handleCreateChildMenu">新增子菜单</menu-context-item>
      </span>
      ...
      >
        ...
        <el-form-item label="是否隐藏">
          <el-select
            clearable
            class="filter-item"
            v-model="resourceFormData.hidden"
            style="width: 100%"
          >
            <el-option
              v-for="item in hideStatusArray"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            ></el-option>
...
export default {
  ...
  data() {
    return {
      ...
      hideStatusArray: [{ value: 0, label: "否" }, { value: 1, label: "是" }],
